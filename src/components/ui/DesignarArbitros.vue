<template>
    <div class="p-6 bg-white rounded-2xl shadow-md">
        <h2 class="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2">
            <UserCheckIcon class="text-green-600" /> {{ designados.length < 4 ? 'Designar Arbitros' : 'Lista de Arbitros'
                }} </h2>

                <!-- Buscar árbitro -->
                <div class="flex gap-3 mb-4" v-if="designados.length < 4">
                    <input v-model="filtro" type="text" placeholder="Buscar por DNI o nombre..."
                ...
                <div v-if="arbitrosEncontrados.length" class="mb-6">
                    ...
                    <ul class="space-y-2">
                        <li v-for="arbitro in arbitrosEncontrados" :key="arbitro.id"
                            class="flex justify-between items-center border rounded-xl p-3 hover:bg-gray-50 transition">
                            <div>
                                <p class="font-medium">{{ arbitro.nombre }} {{ arbitro.apellido }}</p>
                                <p class="text-sm text-gray-500">DNI: {{ arbitro.dni }}</p>
                            </div>

                            <select v-model="rolSeleccionado[arbitro.id]" class="border rounded-xl px-2 py-1 text-sm">
                            ...
                            <button @click="designarArbitro(arbitro)"
                ...
                <div v-if="designados.length">
                    <ul class="space-y-2">
                        <li v-for="a in designados" :key="a.id"
                            class="flex justify-between items-center bg-green-50 border border-green-200 rounded-xl p-3">
                            <span>{{ a.arbitro }} <span class="text-sm font-semibold">({{ a.ci }})</span> <b>{{
                                a.rol.replace("_", " ") }}</b></span>
                            <button @click="quitarArbitro(a)" class="text-red-500 hover:text-red-700 text-sm">
                                Quitar
                            </button>
                        </li>
                    </ul>
...
const buscarArbitro = async () => {
    ...
    try {
        const res = await axios.get(`http://localhost:8080/api/arbitros/buscar?dni=${filtro.value}`)
...
const designarArbitro = async (arbitro) => {
    ...
    let config = {
        ...
        url: 'http://localhost:8080/api/partido/designarArbitros?idPartido=' + props.partidoId,
        headers: {
...
const buscarDesignados = async () => {
    const res = await axios.get("http://localhost:8080/api/designaciones/buscar?idPartido=" + props.partidoId)
...
const quitarArbitro = async (arbitro) => {
    try {
        await axios.delete(`http://localhost:8080/api/partidos/${props.partidoId}/arbitros/${arbitro.id}`)
