    ...
    <div class="flex flex-col max-w-6xl mx-auto p-8">
        <div class="flex items-center justify-evenly p-8 bg-white rounded-t-lg text-lg" v-if="active">
            ...
            <button @click.prevent="buscarTorneo"
                class="cursor-pointer text-white font-bold px-4 py-2 rounded bg-blue-600 hover:bg-blue-800">Buscar</button>
        </div>
        <div class=" bg-gray-100 p-4 rounded-b-lg" v-else>
            <div class="bg-white shadow-lg rounded-lg">
                <div class="border-b border-gray-200 p-4 flex justify-between items-center ">
                    <h1 class="text-2xl font-bold text-gray-800">{{ torneo.nombre }}</h1>
                    <button
                        @click="() => { active = true; torneo = {}, search = '', tipoBusqueda = 'nombre', clubes = [], fixture = [] }"
                        class="bg-gray-100 px-4 py-2 rounded ml-4 text-gray-600 hover:text-gray-800">Volver</button>
                </div>

                <div class="flex border-b border-gray-200">
                    <button @click="activeTab = 'parent'"
                        :class="{ 'bg-gray-100 border-t border-r border-l border-gray-300 rounded': activeTab === 'parent' }"
                        class="px-6 py-3 text-sm font-medium text-gray-700 hover:bg-gray-100 -mb-px">
                        Detalles de la competición
                    </button>
                    <button @click="() => { activeTab = 'clubs'; buscarClubes() }"
                        :class="{ 'bg-gray-100 border-t border-r border-l border-gray-300 rounded': activeTab === 'clubs' }"
                        class="px-6 py-3 text-sm font-medium text-gray-700 hover:bg-gray-100 -mb-px">
                        Clubes
                    </button>
                    <button @click="() => { activeTab = 'fixture'; buscarFixture() }"
                        :class="{ 'bg-gray-100 border-t border-r border-l border-gray-300 rounded': activeTab === 'fixture' }"
                        class="px-6 py-3 text-sm font-medium text-gray-700 hover:bg-gray-100 -mb-px">
                        Fixture del torneo
                    </button>
                </div>
                    ...
                    <div class="flex items-start space-x-6">
                        <div class="w-2/5">
                            ...
                            <form class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" id="num" v-model="torneo.temporada"
                                ...
                                <div class="flex items-center space-x-2">
                                    <div class="flex-1">
                                        <label for="season"
                                            class="block text-sm font-medium text-gray-700">Temporada:</label>
                                        <input type="number" id="season" v-model="torneo.categoria"
                                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">
                                    </div>
                                </div>
                                ...
                                <div>
                                    <label for="shortName" class="block text-sm font-medium text-gray-700">Nombre
                                        corto:</label>
                                    <input type="text" id="shortName" v-model="torneo.shortName"
                                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">
                                </div>

                                <div>
                                    <label for="numClubs" class="block text-sm font-medium text-gray-700">Núm. de
                                        clubes:</label>
                                    <input type="number" id="numClubs" v-model="torneo.numClubs"
                                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">
                                </div>
                        ...
                        <div class="w-3/5">
                            <div class="flex border-b border-gray-200 mb-4">
                                <button
                                    class="px-4 py-2 text-sm font-semibold border-b-2 border-blue-500 text-blue-600 -mb-px">Configuración
                                    general</button>
                                <button
                                    class="px-4 py-2 text-sm font-semibold text-gray-500 hover:text-gray-700">Registros</button>
                                <button
                                    class="px-4 py-2 text-sm font-semibold text-gray-500 hover:text-gray-700">Alineaciones</button>
                            </div>

                            <form class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="teamType" class="block text-sm font-medium text-gray-700">Tipo de
                                        equipo:</label>
                                ...
                                <div>
                                    <label for="disciplines"
                                        class="block text-sm font-medium text-gray-700">Disciplinas:</label>
                                ...
                                <div>
                                    <label for="ageCategory" class="block text-sm font-medium text-gray-700">Categoría
                                        de
                                        edad:</label>
                                ...
                                <div>
                                    <label for="minAge" class="block text-sm font-medium text-gray-700">Edad
                                        mínima:</label>
                                ...
                                <div>
                                    <label for="maxAge" class="block text-sm font-medium text-gray-700">Edad
                                        máxima:</label>
                                ...
                                <div>
                                    <label for="referenceDate" class="block text-sm font-medium text-gray-700">Reference
                                        date:</label>
                                ...
                                <div>
                                    <label for="birthDate" class="block text-sm font-medium text-gray-700">Fecha Nac.
                                        desde:</label>
                ...
                <div v-if="activeTab === 'fixture'">
                    ...
                    <div v-for="(partidos, fecha) in fixture" :key="fecha" class="m-1 mt-2 p-6 bg-gray-100">
                        <!-- Título de la fecha -->
                        ...
                        <div class="mt-5 rounded-lg shadow" v-if="fechaAbierta === fecha">
                            <table class="text-sm text-left border-collapse bg-amber-50">
                                ...
                                <tbody>
                                    <tr v-for="(partido, index) in partidos" :key="partido.idPartido"
                                        class="border-b hover:bg-gray-50">
                                        <td class="px-4 py-2 font-medium">{{ index + 1 }}</td>
                                        <td class="px-4 py-2 font-semibold">{{ partido.local }}</td>
                                        <td class="px-4 py-2 font-semibold">{{ partido.visitante }}</td>
                                        <td class="px-4 py-2 text-gray-600">
                                            {{ partido.fechaCalendario ? partido.fechaCalendario : 'no cargado' }}
                                        </td>
...
const buscarClubes = () => {
    ...
    axios.get("http://localhost:8080/api/club").then((response) => {
...
const buscarTorneo = () => {
    ...
    if (tipoBusqueda.value === 'nombre') {
        try {
            axios.get('http://localhost:8080/api/torneo', {
                params: {
                    nombre: search.value,
                },
                headers: {
        ...
        try {
            axios.get('http://localhost:8080/api/torneo/' + search.value, {
                headers: {
                    'Content-Type': 'application/json',
                },
            }).then((response) => {
...
const buscarFixture = () => {
    axios.get('http://localhost:8080/api/torneo/' + torneo.value.id + '/fixture', {
        headers: {
            'Content-Type': 'application/json',
        },
    }).then((response) => {
...
// Método para abrir/cerrar fechas
const toggleFecha = (fecha) => {
    fechaAbierta.value = fechaAbierta.value === fecha ? null : fecha;
};
