<template>
  <div class="p-6 max-w-5xl mx-auto">
    ...
    <form
      class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white border border-gray-200 rounded-lg shadow-xl p-6"
    >
      <!-- Nombre -->
      <div>
      ...
      <BaseModal
        :show="mostrarModal"
        message="Se registrara el jugador"
        confirmText="Confirmar"
        @confirm="guardarJugador"
        @close="mostrarModal = false"
        classButton="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition"
      />
    </form>
  </div>
</template>
...
const verificarForm = () => {
  if (
    ...
    jugador.value.apellido &&
    jugador.value.dni &&
    jugador.value.fechaNacimiento &&
    jugador.value.nacionalidad &&
    jugador.value.posicion &&
    jugador.value.club
  ) {
